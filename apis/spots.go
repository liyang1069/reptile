...
func CreateSpotsHandler(rw http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	err = Require("longitude", reflect.Float64)
	if err != nil {
		renderError(rw, err, http.StatusBadRequest)
		return
	}
	err = Require("latitude", reflect.Float64)
	if err != nil {
		renderError(rw, err, http.StatusBadRequest)
		return
	}
	//logic
	spot := models.NewSpot(params["title"].(string), params["description"].(string), params["longitude"].(float64), params["latitude"].(float64))
